...
# ── Tool Definitions cho Anthropic API (tool_use) ─────────────────────────────
TOOL_DEFINITIONS = [
    {
        "name": "analyze_excel",
        "description": "Phân tích file Excel trong File_sach/. Trả về tổng đơn, top KTV, top khách hàng, bottleneck công đoạn, remake rate.",
        "input_schema": {
            "type": "object",
            "properties": {
                "file_path": {"type": "string", "description": "Tên file (vd: 0104a2_final.xlsx). Để trống = file mới nhất."},
                "all_files": {"type": "boolean", "description": "True để phân tích tất cả files."}
            }
        }
    },
    {
        "name": "analyze_ktv",
        "description": "Xếp hạng KTV theo số đơn xác nhận, phân tích hiệu suất từng công đoạn.",
        "input_schema": {
            "type": "object",
            "properties": {
                "file_path": {"type": "string", "description": "Tên file Excel. Để trống = file mới nhất."}
            }
        }
    },
    {
        "name": "analyze_customers",
        "description": "Xếp hạng khách hàng (phòng khám) theo số đơn, SL răng, tỉ lệ làm lại.",
        "input_schema": {
            "type": "object",
            "properties": {
                "file_path": {"type": "string", "description": "Tên file Excel. Để trống = file mới nhất."}
    ...
    },
    {
        ...
        "description": "Phân tích lead time trung bình từng công đoạn và từng loại vật liệu.",
        "input_schema": {
            "type": "object",
            "properties": {
                "file_path": {"type": "string", "description": "Tên file Excel. Để trống = file mới nhất."}
    ...
    {
        "name": "get_lab_state",
        "description": "Trạng thái tổng quan xưởng: file dữ liệu mới nhất, số đơn hiện tại.",
        "input_schema": {"type": "object", "properties": {}}
    },
    {
        "name": "list_tasks",
        "description": "Danh sách tasks trong workspace.",
        "input_schema": {
            "type": "object",
            "properties": {
                "project_id": {"type": "string", "description": "Lọc theo project. Để trống = tất cả."},
                "status": {"type": "string", "description": "'pending' hoặc 'done'. Để trống = tất cả."}
            }
        }
    },
    {
        "name": "save_task",
        "description": "Lưu task mới vào workspace.",
        "input_schema": {
            "type": "object",
            "properties": {
                "project_id": {"type": "string", "description": "Project ID (dùng 'default' nếu chưa có)"},
                "description": {"type": "string", "description": "Mô tả task"},
    ...
    {
        "name": "add_learned_fact",
        "description": "Lưu phát hiện/sự thật vào bộ nhớ dài hạn để dùng cho phân tích sau.",
        "input_schema": {
            "type": "object",
            "properties": {
                "fact": {"type": "string", "description": "Nội dung sự thật"},
                "source": {"type": "string", "description": "Nguồn: sandbox / realtime / analysis"},
    ...
    },
    {
        ...
        "description": "Tạo báo cáo tổng hợp, lưu vào analysis/reports/.",
        "input_schema": {
            "type": "object",
            "properties": {
                "report_type": {
                    ...
                    "enum": ["daily_summary", "ktv_report", "customer_report", "lead_time_report"]
                }
            }
        }
    },
    {
        "name": "write_file",
        "description": "Ghi nội dung vào file trong ai_workspace/.",
        "input_schema": {
            "type": "object",
            "properties": {
                "path": {"type": "string", "description": "Đường dẫn file (trong ai_workspace/)"},
                "content": {"type": "string", "description": "Nội dung"},
                "mode": {"type": "string", "description": "overwrite hoặc append", "enum": ["overwrite", "append"]}
            },
...
def build_tool_definitions(registry: dict = None) -> list:
    """Trả về tool definitions cho Anthropic API, lọc theo registry nếu có."""
    if registry is None:
